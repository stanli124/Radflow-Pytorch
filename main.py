...
# from model.radflow_nonetwork import Radflow
from model import Radflow
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for iter, (x, y) in enumerate(train_loader):
            optimizer.zero_grad()
            out = model(x.unsqueeze(-1).to(device), node_neigh)
            out = out * std + mean
            #还原到正常区间
        ...
        with torch.no_grad():
            for iter,(x, y) in enumerate(test_loader):
                pre = model(x.unsqueeze(-1).to(device), node_neigh)
                pre = pre * std + mean
